...
final class BadgeController extends AbstractController
{
    ...
    public function recipients(Request $request, Badge $badge, UserRepository $userRepository, EntityManagerInterface $entityManager, NotificationService $notificationService): Response
    {
            ...
            if ($this->isCsrfTokenValid('recipients'.$badge->getId(), $request->getPayload()->getString('_token'))) {
                ...
                foreach ($newRecipients as $recipient) {
                    $notificationService->notify(
                        $recipient,
                        'Badge debloque',
                        sprintf('Tu as debloque le badge « %s ».', $badge->getName())
                    );
                }
